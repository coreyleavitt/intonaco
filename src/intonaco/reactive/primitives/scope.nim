...
let currentScope {.contextVar.}: Scope = nil
  ## Dynamically-scoped owner that signals, effects, and child tasks
  ## attach themselves to.
  ##
  ## A `ContextVar[Scope]` key backed by chronos's continuation-local
  ## storage (`chronos/contextvars`), so the binding survives every
  ## `await` and concurrent tasks see their own values. Reads are
  ## `currentScope.value`. Writes go through `withScope(s): body` or
  ## `currentScope.withValue(s): body` — both save/restore on every
  ## exit path.
...
proc onCleanup*(body: proc() {.closure.}) {.gcsafe.} =
  ...
  # `.value` read stays outside the cast block (nested cast(gcsafe)
  # defeats the outer one for the statements after it).
  let cs = currentScope.value
  {.cast(gcsafe).}:
    if cs == nil or cs.disposed: return
    cs.cleanups.add body
...
template withScope*(scope: Scope, body: untyped) =
  ...
  ## `currentScope.withValue` (the key's canonical binder); both
