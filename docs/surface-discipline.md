...
- **2026-05-29** — M-ε.3 closed. The static-tier gate (unbaked dep → compile error) is now **unconditional**. The `-d:intonacoStrict` opt-in flag is gone; bindings must either bake their sources via `signals:`/`collections:` or escape via `dynamic name: body`. The relaxed-fallback path (silent runtime composition when no compile-time height) was the discipline's last loophole — removed. The `nimble strictcheck` task was renamed `nimble compileprobes` to reflect that the probes verify unconditional gates, not strict-mode behavior.
